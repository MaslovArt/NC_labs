...
public class MyPersonArrayList {
    ...
    public MyPersonArrayList(int capacity) {
    ...
    public Person[] findAll(MyPredicate predicate) {
        MyPersonArrayList result = new MyPersonArrayList(10);
        for (int i = 0; i < count; i++) {
            if (predicate.test(items[i]))
                result.add(items[i]);
        }
    ...
    public String toString() {
        return MyPersonArrayList.class.getSimpleName() + " [Емкость " + capacity + " размер " + count + "]";
    }
